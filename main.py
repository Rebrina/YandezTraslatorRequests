...
def translate_it(text_for_translate, file_translated, lang, lang_to_translate):
    ...
    a = response.get('text', [])
    with open(file_translated) as f:
        f.write(a[0])
    print(a)
...
if __name__ == "__main__":
    ...
    file_translated = input('Введите путь для сохранения файла перевода')
    lang = input('Введите язык, с которого нужно перевести (например, de,es,fr')
    lang_to_translate =input('Введите язык, на который нужно перевести')
    ...
    a = translate_it(text_for_translate, file_translated, lang, lang_to_translate)
   # print(a)
